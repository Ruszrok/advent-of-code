...
type Test struct {
	divisibleTestValue *big.Int
	SuccessTarget      int
	FailureTatget      int
	ExecutionCount     int
}

func (t *Test) try(worryLevel *big.Int) int {
	t.ExecutionCount++
	var res = big.NewInt(0)
	res = res.Rem(worryLevel, t.divisibleTestValue)
	if res.BitLen() == 0 {
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "  Starting items: ") {
			...
			for _, w := range strings.Split(l, ", ") {
				...
				m.items = append(m.items, big.NewInt(int64(v)))
			}
		}
		...
		if strings.HasPrefix(line, "  Test: divisible by ") {
			...
			t := new(Test)
			t.divisibleTestValue = big.NewInt(int64(v))
			t.ExecutionCount = 0
			m.test = t
		}
...
func dequeue(o []*big.Int) (*big.Int, []*big.Int) {
	if len(o) == 1 {
		return o[0], []*big.Int{}
	}
...
func main() {
	...
	PrintItemsCount(monkeys)

	sort.Slice(monkeys, func(i, j int) bool {
		return monkeys[i].test.ExecutionCount > monkeys[j].test.ExecutionCount
	})

	power := int64(monkeys[0].test.ExecutionCount) * int64(monkeys[1].test.ExecutionCount)
	fmt.Println(power)
}
...
func PrintItemsCount(ms []*Monkey) {
	for i := 0; i < len(ms); i++ {
		fmt.Printf("Monkey %d tested %d items\n", i, ms[i].test.ExecutionCount)
	}
}
